...
public class RankedOrTest extends MatchingSetupTest
{
	...
	*/
	...
	public RankedOrTest(String query, String model)
	{
		this.query = query;
		this.model = model;
	}
	...
	{			
		...
		assertEquals(orDocids.length, docids.size());

		Arrays.sort(orDocids);
		for (int i = 0; i < orDocids.length; i++) {
			assertEquals(orDocids[i], docids.getInt(i));
		}
	}
